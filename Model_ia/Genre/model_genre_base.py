...
# -*- coding: utf-8 -*-
"""
# Modèle 1 : Baseline CNN - Classification de Genre

**Entraînement sur Kaggle**

**Approche :** CNN simple from scratch (pas de transfer learning)
- 3 blocs convolutionnels (32 → 64 → 128 filtres)
...
# %% [code]
"""## 3. Construction du modèle - CNN Baseline"""
...
print("\n" + "=" * 60)
print("ENTRAÎNEMENT - MODÈLE 1 : BASELINE CNN GENRE")
print("=" * 60)
print("Configuration :")
print("  - Architecture : 3 blocs Conv2D (32→64→128)")
...
plt.suptitle('Modèle 1 : Baseline CNN Genre', fontsize=14, y=1.02)
plt.tight_layout()
plt.savefig(os.path.join(OUTPUT_PATH, 'training_curves_genre_base.png'), dpi=150, bbox_inches='tight')
plt.show()
...
plt.title('Matrice de confusion - Modèle 1 : Baseline CNN Genre')
plt.xlabel('Prédit')
plt.ylabel('Réel')
plt.tight_layout()
plt.savefig(os.path.join(OUTPUT_PATH, 'confusion_matrix_genre_base.png'), dpi=150)
plt.show()
...
ax.set_ylabel('Score (%)')
ax.set_title('Performances par classe - Modèle 1 : Baseline CNN Genre')
ax.set_xticks(x_pos)
...
plt.tight_layout()
plt.savefig(os.path.join(OUTPUT_PATH, 'metrics_per_class_genre_base.png'), dpi=150)
plt.show()
...
plt.suptitle('Grad-CAM - Modèle 1 : Baseline CNN Genre\n(Zones chaudes = où le modèle regarde)', fontsize=14, y=1.02)
plt.tight_layout()
plt.savefig(os.path.join(OUTPUT_PATH, 'gradcam_genre_base.png'), dpi=150, bbox_inches='tight')
plt.show()
...
ax.set_title('Distribution de confiance - Modèle 1 : Baseline CNN Genre')
ax.legend()
ax.grid(True, alpha=0.3)

plt.tight_layout()
plt.savefig(os.path.join(OUTPUT_PATH, 'confidence_distribution_genre_base.png'), dpi=150)
plt.show()
...
model.save(os.path.join(OUTPUT_PATH, 'gender_model_base.keras'))
print(f"Modèle sauvegardé : {OUTPUT_PATH}/gender_model_base.keras")
...
tflite_path = os.path.join(OUTPUT_PATH, 'gender_base.tflite')
...
print("=" * 60)
print("RÉSUMÉ FINAL - MODÈLE 1 : BASELINE CNN GENRE")
print("=" * 60)
print(f"""
Architecture :
  - Type : CNN custom from scratch (pas de transfer learning)
  - Bloc 1 : Conv2D(32) → BN → ReLU → MaxPool → Dropout(0.25)
  - Bloc 2 : Conv2D(64) → BN → ReLU → MaxPool → Dropout(0.25)
  - Bloc 3 : Conv2D(128) → BN → ReLU → MaxPool → Dropout(0.25)
...
print("=" * 60)
print("  - gender_model_base.keras")
print("  - gender_base.tflite")
print("  - training_curves_genre_base.png")
print("  - confusion_matrix_genre_base.png")
print("  - metrics_per_class_genre_base.png")
print("  - gradcam_genre_base.png")
print("  - confidence_distribution_genre_base.png")
