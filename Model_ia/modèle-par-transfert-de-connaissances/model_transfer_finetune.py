...
# -*- coding: utf-8 -*-
"""
# Modèle 2 : Fine-tuning progressif - MobileNetV2

**Entraînement sur Kaggle**

**Changement unique vs Modèle 1 :** Fine-tuning des dernières couches du base model
- Phase 1 : entraîner seulement le head (comme Modèle 1)
- Phase 2 : débloquer les 30 dernières couches de MobileNetV2, LR réduit

**Pourquoi :** Les couches hautes de MobileNetV2 apprennent des features de haut niveau
(textures, formes complexes) qui sont spécifiques aux scènes naturelles ImageNet.
En les fine-tunant, on les adapte aux visages et à l'ethnicité.

**Architecture :** Identique au Modèle 1
- MobileNetV2 → GAP → Dense(256) → Dropout(0.4) → Dense(5, softmax)

**Preprocessing :** Identique au Modèle 1
- Images RGB, 128x128, normalisation /255
...
# %% [code]
"""## 3. Construction du modèle - MobileNetV2 Transfer Learning"""
...
model = models.Model(inputs, [age_output, gender_output, ethnicity_output],
                     name='Transfer_MobileNetV2_Finetune')
...
model.summary()
print(f"\nPhase 1 - Paramètres entraînables : {sum(tf.keras.backend.count_params(w) for w in model.trainable_weights):,}")

# %% [code]
"""## 4. Phase 1 : Entraînement du head seul"""
...
print("\n" + "=" * 60)
print("PHASE 1 : ENTRAÎNEMENT DU HEAD SEUL (MobileNetV2 frozen)")
print("=" * 60)
print("  - Learning rate : 0.001")
print("  - Epochs : 30 (minimum 15)")
...
print(f"\nPhase 1 terminée - Accuracy val : {max(history1.history['val_ethnicity_accuracy'])*100:.2f}%")

# %% [code]
"""## 5. Phase 2 : Fine-tuning des dernières couches

**CHANGEMENT CLÉ vs Modèle 1 :** On débloque les 30 dernières couches de MobileNetV2
pour adapter les features de haut niveau aux visages.
"""

# Débloquer les 30 dernières couches
...
print(f"Couches débloquées : 30 dernières couches de MobileNetV2")
...
print("\n" + "=" * 60)
print("PHASE 2 : FINE-TUNING (30 dernières couches débloquées)")
print("=" * 60)
...
print(f"\nPhase 2 terminée - Accuracy val : {max(history2.history['val_ethnicity_accuracy'])*100:.2f}%")
...
# %% [code]
"""## 7. Graphiques d'entraînement (2 phases combinées)"""

# Combiner les historiques des 2 phases
...
plt.suptitle('Modèle 2 : Fine-tuning progressif (Phase 1 → Phase 2)', fontsize=14, y=1.02)
plt.tight_layout()
plt.savefig(os.path.join(OUTPUT_PATH, 'training_curves_transfer_finetune.png'), dpi=150, bbox_inches='tight')
plt.show()

print(f"Phase 1 : {phase1_epochs} epochs (head seul)")
print(f"Phase 2 : {len(history2.history['loss'])} epochs (fine-tuning)")
...
plt.title('Matrice de confusion - Modèle 2 : Fine-tuning progressif')
plt.xlabel('Prédit')
plt.ylabel('Réel')
plt.tight_layout()
plt.savefig(os.path.join(OUTPUT_PATH, 'confusion_matrix_transfer_finetune.png'), dpi=150)
plt.show()
...
ax.set_ylabel('Score (%)')
ax.set_title('Performances par classe - Modèle 2 : Fine-tuning progressif')
ax.set_xticks(x_pos)
...
plt.tight_layout()
plt.savefig(os.path.join(OUTPUT_PATH, 'metrics_per_class_transfer_finetune.png'), dpi=150)
plt.show()
...
plt.suptitle('Grad-CAM - Modèle 2 : Fine-tuning progressif\n(Zones chaudes = où le modèle regarde)', fontsize=14, y=1.02)
plt.tight_layout()
plt.savefig(os.path.join(OUTPUT_PATH, 'gradcam_transfer_finetune.png'), dpi=150, bbox_inches='tight')
plt.show()
...
ax.set_title('Distribution de confiance - Modèle 2 : Fine-tuning progressif')
ax.legend()
ax.grid(True, alpha=0.3)

plt.tight_layout()
plt.savefig(os.path.join(OUTPUT_PATH, 'confidence_distribution_transfer_finetune.png'), dpi=150)
plt.show()
...
model.save(os.path.join(OUTPUT_PATH, 'ethnicity_model_transfer_finetune.keras'))
print(f"Modèle sauvegardé : {OUTPUT_PATH}/ethnicity_model_transfer_finetune.keras")
...
tflite_path = os.path.join(OUTPUT_PATH, 'ethnicity_transfer_finetune.tflite')
...
print("=" * 60)
print("RÉSUMÉ FINAL - MODÈLE 2 : FINE-TUNING PROGRESSIF")
print("=" * 60)
print(f"""
Architecture :
  - Base model : MobileNetV2 (ImageNet)
  - Head : GAP → BN → Dense(256) → Dropout(0.4) → 3 branches (age, gender, ethnicity)
  - Input : RGB 128x128
...
Entraînement :
  ...
  - Loss : Huber(age) + BCE(gender) + CCE(ethnicity)

CHANGEMENT vs Modèle 1 :
  → Fine-tuning des 30 dernières couches (Phase 2)
  → Learning rate réduit à 0.0001 pour le fine-tuning

Résultats :
  - Age MAE : {age_mae:.2f} ans
  - Genre Accuracy : {gender_acc*100:.2f}%
  - Ethnicité Accuracy : {accuracy*100:.2f}%
...
print("=" * 60)
print("  - ethnicity_model_transfer_finetune.keras")
print("  - training_curves_transfer_finetune.png")
print("  - confusion_matrix_transfer_finetune.png")
print("  - metrics_per_class_transfer_finetune.png")
print("  - gradcam_transfer_finetune.png")
print("  - confidence_distribution_transfer_finetune.png")
