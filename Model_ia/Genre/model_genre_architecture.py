...
# -*- coding: utf-8 -*-
"""
# Modèle 2 : Architecture Avancée - ResNet + SE-Net + SeparableConv

**Entraînement sur Kaggle**

**Approche :** CNN avancé avec skip connections, SE-blocks et convolutions séparables
- 4 blocs convolutionnels (32 → 64 → 128 → 256 filtres)
- Classification binaire (Homme/Femme)
- Binary crossentropy + class weights

**Architecture (Functional API) :**
- Bloc 1 : Conv2D(32) → BN → ReLU → SE(ratio=4) + Skip(1x1) → MaxPool → Dropout(0.25)
- Bloc 2 : SeparableConv2D(64) → BN → ReLU → SE(ratio=8) + Skip(1x1) → MaxPool → Dropout(0.25)
- Bloc 3 : SeparableConv2D(128) → BN → ReLU → SE(ratio=8) + Skip(1x1) → MaxPool → Dropout(0.3)
- Bloc 4 : SeparableConv2D(256) → BN → ReLU → SE(ratio=16) + Skip(1x1) → MaxPool → Dropout(0.3)
- GAP → Dense(256) → Dropout(0.5) → Dense(1, sigmoid)

**Améliorations vs Modèle 1 :**
- ResNet (Skip Connections) : meilleur flux de gradient
- SE-Net (Squeeze-and-Excitation) : attention sur les channels
- SeparableConv2D : moins de paramètres, meilleure généralisation
- 4 blocs au lieu de 3
...
# %% [code]
"""## 3. Construction du modèle - Architecture Avancée (ResNet + SE-Net + SeparableConv)"""
...
def build_model(input_shape=(IMG_SIZE, IMG_SIZE, 3)):
    """
    Architecture Avancée : ResNet + SE-Net + SeparableConv
    ...
    Modifications vs Modèle 1 (Baseline) :
    - Conv2D → SeparableConv2D (sauf bloc 1)
    - Ajout SE-Block après chaque conv (attention sur les channels)
    - Ajout Skip Connection autour de chaque bloc (meilleur flux de gradient)
    - 4 blocs au lieu de 3
    """
    ...
    x = se_block(x, ratio=4)  # +SE : Attention sur les channels

    # Skip connection : adapter l'input à 32 channels
    shortcut = layers.Conv2D(32, (1, 1), padding='same')(inputs)
    x = layers.Add()([x, shortcut])  # +ResNet : Skip connection
    ...
    x = layers.SeparableConv2D(64, (3, 3), padding='same')(x)  # +Separable
    x = layers.BatchNormalization()(x)
    x = layers.Activation('relu')(x)
    x = se_block(x, ratio=8)  # +SE

    # Skip connection : adapter 32 → 64 channels
    shortcut = layers.Conv2D(64, (1, 1), padding='same')(shortcut)
    x = layers.Add()([x, shortcut])  # +ResNet
    ...
    x = layers.SeparableConv2D(128, (3, 3), padding='same')(x)  # +Separable
    x = layers.BatchNormalization()(x)
    x = layers.Activation('relu')(x)
    x = se_block(x, ratio=8)  # +SE
    ...
    x = layers.SeparableConv2D(256, (3, 3), padding='same')(x)  # +Separable
    x = layers.BatchNormalization()(x)
    x = layers.Activation('relu')(x)
    x = se_block(x, ratio=16)  # +SE
    ...
    model = Model(inputs, outputs, name='CNN_Genre_Architecture')
    return model
...
print("\n" + "=" * 60)
print("ENTRAÎNEMENT - MODÈLE 2 : ARCHITECTURE AVANCÉE GENRE")
print("=" * 60)
print("Configuration :")
...
plt.suptitle('Modèle 2 : Architecture Avancée Genre', fontsize=14, y=1.02)
plt.tight_layout()
plt.savefig(os.path.join(OUTPUT_PATH, 'training_curves_genre_architecture.png'), dpi=150, bbox_inches='tight')
plt.show()
...
plt.title('Matrice de confusion - Modèle 2 : Architecture Avancée Genre')
plt.xlabel('Prédit')
plt.ylabel('Réel')
plt.tight_layout()
plt.savefig(os.path.join(OUTPUT_PATH, 'confusion_matrix_genre_architecture.png'), dpi=150)
plt.show()
...
ax.set_ylabel('Score (%)')
ax.set_title('Performances par classe - Modèle 2 : Architecture Avancée Genre')
ax.set_xticks(x_pos)
...
plt.tight_layout()
plt.savefig(os.path.join(OUTPUT_PATH, 'metrics_per_class_genre_architecture.png'), dpi=150)
plt.show()
...
def make_gradcam_heatmap(img_array, model, pred_index=None):
    """
    Génère une heatmap Grad-CAM pour un CNN avec architecture avancée.
    Supporte Conv2D et SeparableConv2D comme dernière couche convolutionnelle.
    """
    # Trouver la dernière couche convolutionnelle (Conv2D ou SeparableConv2D)
...
plt.suptitle('Grad-CAM - Modèle 2 : Architecture Avancée Genre\n(Zones chaudes = où le modèle regarde)', fontsize=14, y=1.02)
plt.tight_layout()
plt.savefig(os.path.join(OUTPUT_PATH, 'gradcam_genre_architecture.png'), dpi=150, bbox_inches='tight')
plt.show()
...
ax.set_title('Distribution de confiance - Modèle 2 : Architecture Avancée Genre')
ax.legend()
ax.grid(True, alpha=0.3)

plt.tight_layout()
plt.savefig(os.path.join(OUTPUT_PATH, 'confidence_distribution_genre_architecture.png'), dpi=150)
plt.show()
...
model.save(os.path.join(OUTPUT_PATH, 'gender_model_architecture.keras'))
print(f"Modèle sauvegardé : {OUTPUT_PATH}/gender_model_architecture.keras")
...
tflite_path = os.path.join(OUTPUT_PATH, 'gender_architecture.tflite')
...
print("=" * 60)
print("RÉSUMÉ FINAL - MODÈLE 2 : ARCHITECTURE AVANCÉE GENRE")
print("=" * 60)
print(f"""
Architecture :
  - Type : CNN avancé avec ResNet + SE-Net + SeparableConv (Functional API)
  - Bloc 1 : Conv2D(32) → BN → ReLU → SE(ratio=4) + Skip(1x1) → MaxPool → Dropout(0.25)
  - Bloc 2 : SeparableConv2D(64) → BN → ReLU → SE(ratio=8) + Skip(1x1) → MaxPool → Dropout(0.25)
  - Bloc 3 : SeparableConv2D(128) → BN → ReLU → SE(ratio=8) + Skip(1x1) → MaxPool → Dropout(0.3)
  - Bloc 4 : SeparableConv2D(256) → BN → ReLU → SE(ratio=16) + Skip(1x1) → MaxPool → Dropout(0.3)
...
Améliorations architecturales vs Modèle 1 (Baseline) :
  - Skip Connections (ResNet) : meilleur flux de gradient, entraînement plus stable
  - SE-Blocks (Squeeze-and-Excitation) : attention sur les channels importants
  - SeparableConv2D : réduction des paramètres, meilleure généralisation
  - 4 blocs convolutionnels au lieu de 3 (ajout bloc 256 filtres)
...
print("=" * 60)
print("  - gender_model_architecture.keras")
print("  - gender_architecture.tflite")
print("  - training_curves_genre_architecture.png")
print("  - confusion_matrix_genre_architecture.png")
print("  - metrics_per_class_genre_architecture.png")
print("  - gradcam_genre_architecture.png")
print("  - confidence_distribution_genre_architecture.png")
